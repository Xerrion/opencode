...
Commit messages and PR descriptions are not yours - they belong to the `git` agent. User-facing error strings live in code and belong to the `software-engineer` agent. `AGENTS.md` files are meta-configuration, not documentation, and are owned elsewhere.
