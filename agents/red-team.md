...
3. **Enumerate plausible attacks.** For each input and boundary, list the attack classes from `<attack_surface>` that are actually reachable from this code. Discard classes that have no path here.
