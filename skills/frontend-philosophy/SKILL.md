...
- **NEVER** Inter, Roboto, Arial, or the platform's default font stack. Pick something with personality.
- MUST pair a dramatic display font with a refined body font.
- Loading states, empty states, and error views MUST use the same type system - no fallback to platform defaults.

### 2. Committed Color & Theme

- **NEVER** evenly-weighted rainbow palettes or "blue/white/gray" defaults. MUST have one dominant hue.
- Establish a design-token system early (e.g. CSS variables on the web). Kill the "purple gradient on white" AI cliche.
...
- Use the platform's native animation primitives for simple motion and a dedicated animation library for orchestrated sequences (e.g. CSS animations for plain HTML, Motion for React). Prioritize staggered reveals and surprising hover states.
- Loading spinners and skeletons count as motion - they MUST be styled, not default platform/library chrome.

### 4. Brave Spatial Composition

- **NEVER** sprinkle the styling system's default spacing steps everywhere (e.g. Tailwind's `p-4`, `gap-4`). Every spacing value MUST be a deliberate choice.
...
- [ ] **Space:** Can you justify every spacing value, or did you copy-paste the default spacing step?
