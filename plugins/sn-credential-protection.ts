...
const CREDENTIAL_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  { pattern: /servicenow_password\s*=\s*["'][^"']+["']/i, label: "ServiceNow password" },
  { pattern: /servicenow_username\s*=\s*["'][^"']+["']/i, label: "ServiceNow username" },
  { pattern: /SERVICENOW_PASSWORD\s*=\s*\S+/i, label: "ServiceNow password env var" },
  { pattern: /https?:\/\/[^:]+:[^@]+@[^.]+\.service-now\.com/i, label: "ServiceNow URL with embedded credentials" },
  { pattern: /sn_password\s*[:=]\s*["'][^"']+["']/i, label: "SN password assignment" },
  { pattern: /glide\.servlet\.uri.*password/i, label: "Glide servlet credential" },
]
...
export const SnCredentialProtection: Plugin = async ({ client }) => {
  const log = (level: "info" | "warn" | "error", message: string) =>
    client.app.log({ body: { service: "sn-credential-protection", level, message } })
    ...
    "tool.execute.before": async (input: { tool: string; args?: Record<string, unknown> }) => {
      ...
      if (input.tool === "write" || input.tool === "edit") {
        ...
        if (filePath && isEnvFile(filePath)) {
          await log("warn", `Writing to env file: ${filePath} -- ensure this file is in .gitignore`)
        }
      }
