...
const NON_PROD_SUFFIXES = ["-dev", "-test", "-sandbox", "-uat", "-staging", "-qa", "-demo", "-training", "-lab"]
...
function isProductionInstance(): { isProd: boolean; reason: string } {
  ...
  if (envFlag && envFlag !== "production" && envFlag !== "prod") {
    return { isProd: false, reason: `SERVICENOW_ENV=${envFlag} (non-production)` }
  }

  // Infer from instance URL
  const instanceUrl = (process.env.SERVICENOW_INSTANCE_URL || process.env.servicenow_instance_url || "").toLowerCase()
  if (!instanceUrl) {
  ...
  const isNonProd = NON_PROD_SUFFIXES.some((suffix) => subdomain.endsWith(suffix))

  if (isNonProd) {
    return { isProd: false, reason: `Instance subdomain '${subdomain}' contains non-prod suffix` }
  }
...
export const SnProductionWarning: Plugin = async ({ client }) => {
  const log = (level: "info" | "warn" | "error", message: string) =>
    client.app.log({ body: { service: "sn-production-warning", level, message } })

  // Check environment on plugin load
  const { isProd, reason } = isProductionInstance()
  if (isProd) {
    await log("warn", `Production environment detected (${reason}). Write operations will be BLOCKED.`)
  } else {
    await log("info", `Non-production environment (${reason}). Write operations allowed.`)
  }
