...
- **MUST** commit after every implementation cycle - atomic, conventional message - so each cycle is separately revertable and the final review has a clean trail. Push and PR creation wait until the final review approves.
- **MUST** self-review the complete diff before committing and before requesting the final review: re-read changed files, inspect error and security paths, sweep renamed or reshaped references, and confirm verification evidence. Sending an unreviewed diff spends a reviewer's attention on defects you could have caught yourself.
- **MUST** request one independent `reviewer` review when the work is complete, covering the full accumulated change set - code, tests, configuration, and documentation alike. Agents that cannot delegate (no `task` permission), or whose only writes are their own deliverable artefacts under `.deliverables/`, instead return the exact changed-file list and self-review evidence to their caller, who then owns routing the review.
- **MUST NOT** report a file-changing task complete until that final review returns `APPROVE` - except under the carve-out above, where completion means the changed-file list and evidence were handed to the caller. On `REQUEST_CHANGES`, address every applicable BLOCKER as new commits, repeat verification and self-review, then request review again. On `NEEDS_DISCUSSION`, put the concrete decision to the user.
